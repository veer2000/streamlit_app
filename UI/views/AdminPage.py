                ...
                with st.container(border=True, height=70, width=900, horizontal_alignment='center'):
                    ...
                    with content_dropdown:
                        ...
                        with dropdown:
                            ...
                            st.selectbox(options=user_drop_down_list, label="User",label_visibility="collapsed", key="user")
                ...
                with st.container(border=True, height=70, width=900, horizontal_alignment='center'): #border=True,
                    ...
                    with content_dropdown:
                        ...
                        with dropdown:
                            ...
                            st.selectbox(options=priority_drop_down_list, label="Priority1",label_visibility="collapsed",key="priority1")
                with st.container(border=True, height=70, width=900, horizontal_alignment='center'):
                    content_user, content_dropdown, _ = st.columns([0.5,0.5,0.1])
                    with content_user:
                        st.write("Priority 2")
                    with content_dropdown:
                        ...
                        with dropdown:
                            ...
                            st.selectbox(options=priority_drop_down_list, label="Priority2",label_visibility="collapsed", key="priority2")
                with st.container(border=True, height=70, width=900, horizontal_alignment='center'):
                    content_user, content_dropdown, _ = st.columns([0.5,0.5,0.1])
                    with content_user:
                        st.write("Priority 3")
                    with content_dropdown:
                        ...
                        with dropdown:
                            ...
                            st.selectbox(options=priority_drop_down_list, label="Priority3",label_visibility="collapsed", key="priority3")
            with st.container(border=False, height=70, width=900):
                _, button_box,_ = st.columns([1,1,1])
                with button_box:
                    if st.button("Submit", use_container_width=True):
                        # 3. On submit, pull the values into your dictionary
                        st.session_state.admin_form_data = {
                            "user_id": st.session_state.get("id"),  # From your login
                            "contact_method": st.session_state.user,
