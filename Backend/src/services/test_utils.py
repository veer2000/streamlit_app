...
# from src.services.access_emails import get_allocated_email
# from src.services.integrated_Agent_for_emailDraft import get_draft_email
...
## ***********************************************************************************************************##
# original code for card1 - testing eamil
# In utils.py
def view_email(email):
    ...
    try:
        ...
        with st.container(border=True):
            ...
            with col1:
                return None
                # st.markdown(f"**From:** `{email['from']['emailAddress']['name']}`")
                # st.markdown(f"**Subject:** {email['subject']}")
                # st.markdown(f"**Msg_id:** {email['id']}")
            with col2:
                return None
                # st.markdown(f"**Time:** `{email['receivedDateTime']}`")
