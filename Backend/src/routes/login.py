...
async def loginUser(email:str, password: str,): # request : Annotated[dict, Depends(get_db)]
    try:
        hash_generate = hash_pass(password)
        print(hash_generate)
        #NOTE: to validate_password params should be plantext as type bytes and hashed as type bytes
        if validate_password(password.encode('utf-8'), hash_generate):
            return True
        else:
            return False
        ...
        #TODO: using email get user details and validte
        # print(request)
